...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        # Split on sentence boundaries, keeping punctuation with each sentence
        parts = re.split(r'(?<=[.!?]) +|(?<=\.)\n', text)
...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        if not remaining_separators:
            # No separator left; return text as-is even if oversized
            return [current_text]
        ...
        if separator == "":
            # Character-level fallback: slice into chunk_size pieces
            return [
        ...
        # Recursively split any part that still exceeds chunk_size
        all_pieces: list[str] = []
        for part in parts:
            if len(part) <= self.chunk_size:
                all_pieces.append(part)
            else:
                all_pieces.extend(self._split(part, next_separators))

        # Greedily merge adjacent small pieces back together
        result: list[str] = []
        current = ""
